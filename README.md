...
In this example we utilize the power of the "strongly typed" Datasets by providing the schema as the case class CWord. Thus, instead of using type agnostic Rows, we use Scala’s case classes to describe the contents of the rows and access the attribute - in this case "Value". Note that the case class CWord here should have a column name 'Value'. This way you will not get a run-time error in Spark because your CWord column name is identical to the default column name 'Value' generated by Spark on run-time.

In case you wish to use the “where” operator with an SQL style predicate you can then write the where clause as follows.
